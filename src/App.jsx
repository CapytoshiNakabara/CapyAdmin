...
function App() {
  ...
  const logValues = async () => {
    ...
    console.log(allres)
    const distinctAddresses = new Set(allres.result.map((res) => { return res.to }))
    console.log(distinctAddresses);
    exportToCsv(distinctAddresses)

    setIsFetching(false)
  }

  const exportToCsv = (arrayToCsv) => {

    let CsvString = "Address";
    CsvString += "\r\n";
    arrayToCsv.forEach((item, index) => {
      CsvString += item
      CsvString += "\r\n";
    })
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <input id="fromDate" type='date' placeholder='Från' onChange={(e) => setFromDate(e.target.value)} />
        ...
        <input id="toDate" type='date' placeholder='Till' onChange={(e) => setToDate(e.target.value)} />
        ...
        <input id="tokenAddress" placeholder='Token address' value={tokenAddress} onChange={(e) => setTokenAddress(e.target.value)} />
        <br />
        <label htmlFor="#fromAddress">From address (Has to be a liquidity pool): </label>
        <input id="fromAddress" placeholder='From address' value={fromAddress} onChange={(e) => setFromAddress(e.target.value)} />
        <br />
